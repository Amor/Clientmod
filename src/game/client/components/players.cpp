	...
	)
{
	...
	if (config.cl_antiping && 
		...
		player_char && player_char->player_state == PLAYERSTATE_PLAYING)
	{	
		...
		vec2 prev = gameclient.clients[player_info->cid].previous_prediction;
		...
		gameclient.clients[player_info->cid].previous_prediction = shadow_position;

		TEE_RENDER_INFO shadow = render_info;
		float color_body_mix = (shadow.color_body.r + shadow.color_body.g + shadow.color_body.b) / 2.5f;
